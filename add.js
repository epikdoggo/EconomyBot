...
module.exports = {
    data: {
        name: 'staffadditem',
        description: "Manually add an item or balance to another user's account (Admin only)",
        options: [
            {
                name: 'target-user',
                description: 'The user to add the item or balance to',
                type: ApplicationCommandOptionType.User,
                required: true,
            },
            {
                name: 'item-type',
                description: 'The type of item to add or "coins" for balance',
            ...
            {
                name: 'amount',
                description: 'The amount of the item or balance to add',
    ...
    run: async ({ interaction, client }) => {
        ...
        if (amount <= 0) {
            return interaction.reply('Please specify a valid amount greater than 0.');
        }
        ...
        try {
            ...
            if (item === 'coins') {
                ...
                userProfile.inventory[item] = (userProfile.inventory[item] || 0) + amount;
            }
            ...
            const embed = new EmbedBuilder()
                .setColor('#00FF00')
                .setTitle('Item or Balance Added')
                .setDescription(`Successfully added ${amount} ${item}(s) to <@${targetUser.id}>.`)
            ...
            console.error(`Error handling /staffadditem: ${error}`);
